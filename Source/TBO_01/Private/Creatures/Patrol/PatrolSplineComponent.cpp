...
#include "Creatures/Patrol/PatrolSplineComponent.h"
#include "Creatures/Patrol/PatrolPath.h"
...
bool UPatrolSplineComponent::IncrementIndexAndSendScriptedActionEvent()
{
	...
	Command.GetInterface()->IncrementIndex(AssignedPatrolPath->Spline,CurrentIndex);
	AScriptedPatrolPath* ScriptedPath = Cast<AScriptedPatrolPath>(AssignedPatrolPath);
	if (ScriptedPath)
	{
		if (PreviousIndex != CurrentIndex)
		{
			ScriptedPath->IsCompleted = false;
			const FScriptedAction& Action = ScriptedPath->GetAction(PreviousIndex);

			if (OnScriptedAction.IsBound())
			{
				OnScriptedAction.Broadcast(Action);
			}
			
			return true;
		}
		
		ScriptedPath->IsCompleted = true;
